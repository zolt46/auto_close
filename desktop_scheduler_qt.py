...
class IdleTimeDetector:
    ...
    def seconds_since_last_input(self) -> float:
        if self._last_input_info is None or self._ctypes is None:
            return 0.0
        try:
            info = self._last_input_info()
            info.cbSize = self._ctypes.sizeof(self._last_input_info)
            if not self._user32.GetLastInputInfo(self._ctypes.byref(info)):
                return 0.0
            elapsed_ms = int(self._kernel32.GetTickCount64()) - int(info.dwTime)
...
class SchedulerConfig:
    ...
    wakeup_audio_enabled: bool = True
    wakeup_audio_urls: List[str] = field(default_factory=list)
    wakeup_audio_mode: str = "fullscreen"
    wakeup_target_enabled: bool = True
    wakeup_target_url: str = ""
    wakeup_target_mode: str = "kiosk"
    wakeup_idle_to_show_sec: float = 10.0
    wakeup_active_threshold_sec: float = 1.0
    wakeup_poll_sec: float = 0.5
    wakeup_chrome_relaunch_cooldown_sec: float = 10.0
    wakeup_chrome_repeat: bool = True
    wakeup_saver_enabled: bool = True
    wakeup_saver_image_mode: str = "bundled"
    wakeup_image_path: Optional[str] = None
    ...
    def from_dict(data: Dict[str, object]) -> "SchedulerConfig":
        ...
        for key in (
            ...
            "shutdown_logs",
            "wakeup_enabled",
            "wakeup_audio_enabled",
            "wakeup_audio_urls",
            "wakeup_audio_mode",
            "wakeup_target_enabled",
            "wakeup_target_url",
            "wakeup_target_mode",
            "wakeup_idle_to_show_sec",
            "wakeup_active_threshold_sec",
            "wakeup_poll_sec",
            "wakeup_chrome_relaunch_cooldown_sec",
            "wakeup_chrome_repeat",
            "wakeup_saver_enabled",
            "wakeup_saver_image_mode",
            "wakeup_image_path",
            "user_password_hash",
            "admin_password_hash",
        ):
        ...
        if base.wakeup_image_path and not isinstance(base.wakeup_image_path, str):
            base.wakeup_image_path = None
        try:
            base.audio_volume = float(base.audio_volume)
        except (TypeError, ValueError):
            base.audio_volume = 0.9
        base.audio_volume = max(0.0, min(1.0, base.audio_volume))
        base.wakeup_enabled = coerce_bool(data.get("wakeup_enabled", base.wakeup_enabled), base.wakeup_enabled)
        base.wakeup_chrome_repeat = coerce_bool(
            data.get("wakeup_chrome_repeat", base.wakeup_chrome_repeat),
            base.wakeup_chrome_repeat,
        )
        base.wakeup_saver_enabled = coerce_bool(
            data.get("wakeup_saver_enabled", base.wakeup_saver_enabled),
            base.wakeup_saver_enabled,
        )
        if not isinstance(base.wakeup_saver_image_mode, str) or not base.wakeup_saver_image_mode:
            base.wakeup_saver_image_mode = "bundled"
        if not isinstance(base.wakeup_image_path, str) and base.wakeup_image_path is not None:
            base.wakeup_image_path = None
        try:
            base.wakeup_idle_to_show_sec = float(base.wakeup_idle_to_show_sec)
        except (TypeError, ValueError):
            base.wakeup_idle_to_show_sec = 10.0
        try:
            base.wakeup_active_threshold_sec = float(base.wakeup_active_threshold_sec)
        except (TypeError, ValueError):
            base.wakeup_active_threshold_sec = 1.0
        try:
            base.wakeup_poll_sec = float(base.wakeup_poll_sec)
        except (TypeError, ValueError):
            base.wakeup_poll_sec = 0.5
        try:
            base.wakeup_chrome_relaunch_cooldown_sec = float(base.wakeup_chrome_relaunch_cooldown_sec)
        except (TypeError, ValueError):
            base.wakeup_chrome_relaunch_cooldown_sec = 10.0
        base.wakeup_audio_enabled = coerce_bool(
            data.get("wakeup_audio_enabled", base.wakeup_audio_enabled),
            base.wakeup_audio_enabled,
        )
        base.wakeup_target_enabled = coerce_bool(
            data.get("wakeup_target_enabled", base.wakeup_target_enabled),
            base.wakeup_target_enabled,
        )
        audio_urls = data.get("wakeup_audio_urls")
        if isinstance(audio_urls, list):
            base.wakeup_audio_urls = [str(item).strip() for item in audio_urls if str(item).strip()]
        else:
            legacy_audio = data.get("wakeup_youtube_url") or ""
            if isinstance(legacy_audio, str) and legacy_audio.strip():
                base.wakeup_audio_urls = [legacy_audio.strip()]
        base.wakeup_audio_mode = str(data.get("wakeup_audio_mode") or base.wakeup_audio_mode or "fullscreen")
        if base.wakeup_audio_mode not in {"normal", "fullscreen", "kiosk"}:
            base.wakeup_audio_mode = "fullscreen"
        base.wakeup_target_url = str(data.get("wakeup_target_url") or data.get("wakeup_kiosk_url") or base.wakeup_target_url or "")
        base.wakeup_target_mode = str(data.get("wakeup_target_mode") or base.wakeup_target_mode or "kiosk")
        if base.wakeup_target_mode not in {"normal", "fullscreen", "kiosk"}:
            base.wakeup_target_mode = "kiosk"
        return base
...
def is_service_day(cfg: SchedulerConfig, day_cfg: DaySchedule, current_date: date) -> bool:
    if not day_cfg.enabled:
        return False
    if cfg.holidays_enabled and is_holiday(cfg, current_date):
        return False
...
class StyledToggle(QtWidgets.QCheckBox):
    ...
    def paintEvent(self, event: QtGui.QPaintEvent) -> None:  # pragma: no cover - UI 이벤트
        ...
        track_off = palette.color(QPalette.Light)
...
class DayCard(FancyCard):
    ...
    def set_weekend_locked(self, locked: bool) -> None:
        widgets = (
            self.enable_chk,
            self.auto_chk,
            self.time_edit,
        ...
        if locked:
            self.enable_chk.setToolTip("주말 자동 제외가 활성화되어 일정을 사용할 수 없습니다.")
            self.auto_hint.setText("주말 자동 제외가 활성화되어 있습니다")
            self.auto_hint.setVisible(True)
        else:
            self.enable_chk.setToolTip("이 요일의 일정을 활성화하거나 비활성화합니다.")
            self._update_mode()
...
class HolidayPanel(FancyCard):
    ...
    def _persist(self) -> None:
        ...
        def updater(cfg: SchedulerConfig) -> None:
            ...
            if enabled and skip_weekends:
                for day_key in ("sat", "sun"):
                    if day_key in cfg.days:
                        cfg.days[day_key].enabled = False
...
class SettingsPanel(FancyCard):
    ...
    def __init__(self, cfg_mgr: ConfigManager, accent: str, parent=None) -> None:
        ...
        self.holiday_delay_spin = QtWidgets.QSpinBox()
        self.holiday_delay_spin.setRange(0, 300)
        self.holiday_delay_spin.setValue(cfg_mgr.config.holiday_shutdown_delay)
        self.holiday_delay_spin.setToolTip("휴일 자동 종료 시 지연 시간을 설정합니다.")
        ...
        general_form.addRow("비밀번호 관리", password_row)
        outer.addLayout(general_form)

        schedule_group = QtWidgets.QGroupBox("스케줄 종료 정책")
        schedule_form = QtWidgets.QFormLayout(schedule_group)
        schedule_form.setSpacing(10)
        schedule_form.addRow("원격 종료", create_toggle_field("사용", self.remote_toggle))
        schedule_form.addRow("본체 종료", create_toggle_field("사용", self.local_toggle))
        schedule_form.addRow("종료 지연(초)", self.delay_spin)
        outer.addWidget(schedule_group)

        holiday_group = QtWidgets.QGroupBox("휴일 종료 정책")
        holiday_form = QtWidgets.QFormLayout(holiday_group)
        holiday_form.setSpacing(10)
        holiday_form.addRow("휴일 자동 종료", create_toggle_field("사용", self.holiday_shutdown_toggle))
        holiday_form.addRow("휴일 원격 종료", create_toggle_field("사용", self.holiday_remote_toggle))
        holiday_form.addRow("휴일 본체 종료", create_toggle_field("사용", self.holiday_local_toggle))
        holiday_form.addRow("휴일 종료 지연(초)", self.holiday_delay_spin)
        outer.addWidget(holiday_group)
        ...
        self.holiday_delay_spin.valueChanged.connect(lambda _: self._persist())
    ...
    def _persist(self) -> None:
        ...
        def updater(cfg: SchedulerConfig) -> None:
            ...
            cfg.holiday_shutdown_delay = self.holiday_delay_spin.value()
        self.cfg_mgr.update(updater)
        set_startup(start)
    ...
    def sync_from_config(self) -> None:
        ...
        self.holiday_delay_spin.blockSignals(True)
        self.holiday_delay_spin.setValue(cfg.holiday_shutdown_delay)
        self.holiday_delay_spin.blockSignals(False)
        self._load_hosts()
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, cfg_mgr: ConfigManager, brand_icon: Optional[QIcon] = None) -> None:
        ...
        self._secret_timer = QtCore.QElapsedTimer()
    ...
    def _build_ui(self) -> None:
        ...
        self.drawer.setParent(root)
        self.drawer.raise_()
        self._drawer_overlay = DrawerOverlay(root, self._hide_drawer)
    ...
    def _show_drawer(self) -> None:
        ...
        if getattr(self, "top_bar", None):
            self.top_bar.raise_()
        self.drawer.raise_()
        self._update_menu_button_icon(True)
    ...
    def _update_drawer_geometry(self) -> None:
        ...
        if root is None:
            return
        drawer_rect = QtCore.QRect(0, 0, self.drawer_width, root.height())
        self.drawer.setGeometry(drawer_rect)
        if self._drawer_overlay:
            self._drawer_overlay.setGeometry(root.rect())

    def _update_menu_button_icon(self, opened: bool) -> None:
        size = QtCore.QSize(28, 28)
        pixmap = QtGui.QPixmap(size)
        pixmap.fill(Qt.transparent)
        painter = QtGui.QPainter(pixmap)
        painter.setRenderHint(QtGui.QPainter.Antialiasing)
        painter.setPen(QtGui.QPen(QtGui.QColor("#1C2B3E")))
        font = _build_ui_font(18, QFont.Weight.Bold)
        painter.setFont(font)
    ...
    def _execute_startup_shutdowns(self) -> None:
        ...
        if allow_local:
            holiday_delay = cfg.holiday_shutdown_delay
            self._append_shutdown_log(
                "본체 종료",
                f"{reason} 자동 종료 - {holiday_delay}초 후 종료",
            )
            threading.Thread(target=shutdown_local, args=(holiday_delay,), daemon=True).start()
    ...
    def _on_config_changed(self, cfg: SchedulerConfig) -> None:
        ...
        self._apply_weekend_locks(cfg)
        self._update_today_summary()
        self.log_card.update_logs(cfg.shutdown_logs)
        self.scheduler._compute_next_run()

    def _apply_weekend_locks(self, cfg: SchedulerConfig) -> None:
        lock_weekends = cfg.holidays_enabled and cfg.auto_skip_weekends
        for day_key, card in self.day_cards.items():
            if day_key in {"sat", "sun"}:
                card.set_weekend_locked(lock_weekends)
            else:
                card.set_weekend_locked(False)
